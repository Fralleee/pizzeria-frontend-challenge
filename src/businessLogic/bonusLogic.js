...
export const getBonusText = (bonus) =>
  `You have received ${bonus.deductionPercentage * 100}% bonus`
export const getNextAvailableBonusText = (amount, bonus) =>
  bonus &&
  `Spend $${-(Math.round((amount - bonus.threshold) * 100) / 100).toFixed(
    2
  )} more to receive ${bonus.deductionPercentage * 100}% bonus`
...
export const calculateBonus = (order) => {
  ...
  const amount = order.reduce(
    (acc, curr) => (acc += curr.cost * curr.amount),
    0
  )
